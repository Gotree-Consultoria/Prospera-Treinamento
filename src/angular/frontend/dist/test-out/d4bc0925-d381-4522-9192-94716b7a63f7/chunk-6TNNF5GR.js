    ...
    AuthService = class AuthService2 {
      ...
      login(payload) {
        return this.api.post("/auth/login", payload, { withCredentials: true }).pipe(tap((response) => this.persistSession(response)), switchMap((response) => {
          if (response.profile || response.user) {
            ...
            this.userSubject.next(__spreadProps(__spreadValues({}, profile), { email: profile.email ?? response.email ?? payload.email }));
      ...
      fetchProfile(options = {}) {
        ...
        return this.api.get("/profile/me").pipe(tap((profile) => {
          const email = profile.email ?? this.getStoredEmail();
          this.userSubject.next(__spreadProps(__spreadValues({}, profile), { email }));
        }), catchError((error) => {
          this.clearSession();
      ...
      updateProfile(changes) {
        return this.api.patch("/profile/me", changes).pipe(tap((profile) => {
          const email = profile.email ?? this.getStoredEmail();
          this.userSubject.next(__spreadProps(__spreadValues({}, profile), { email }));
        }));
      }
      ...
      persistSession(response) {
        ...
        try {
          ...
          const email = response.email ?? this.userSubject.value?.email;
          if (email) {
...
@angular/router/fesm2022/router.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-6TNNF5GR.js.map
