...
describe("CatalogService", () => {
  ...
  it("combina produtos, pacotes e cat\xE1logo p\xFAblico sem duplicar itens", (done) => {
    apiService.get.and.callFake((path) => {
      switch (path) {
        case "/api/products":
          return of([{ id: 1, title: "E-book", description: "A", format: "EBOOK" }]);
        case "/api/packages":
          return of([{ id: 1, title: "Combo SST", description: "B", format: "PACKAGE" }]);
        case "/public/catalog?type=EBOOK":
          return of({ items: [{ id: 2, title: "Checklist", description: "C", type: "EBOOK" }] });
        default:
          return of([]);
      }
    });
